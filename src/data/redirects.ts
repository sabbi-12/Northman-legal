...
// Explicit 1:1 redirects for all 56 live WordPress blog posts, verified
// against the real WP site (root-level "Post name" permalinks — no /blog/
// or /news-updates/ prefix on WP) and cross-checked against the Sanity
// slugs from the migration (2026-08-07). Two posts share the visually
// similar "Qatar announced New Residence Permit..." title but are genuinely
// different posts/URLs on WP — both got their own entry, not deduped.
...
export const allRedirects: RedirectRule[] = [...staticRedirects, ...blogPostRedirects, ...patternRedirects];
